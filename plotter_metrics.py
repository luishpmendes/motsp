...
fig.suptitle("Multi-Objective Travelling Salesman Problem", fontsize = 25)
for i in range(len(solvers)):
    for j in range(len(metrics)):
        ...
        for l in range(len(metrics)):
            if j != l:
                ...
                axs[j][l].scatter(x = metrics[l][i], y = metrics[j][i], color = colors[i], label = solver_labels[solvers[i]], marker = (i + 3, 2, 0), alpha = 0.60)
                if axs[j][l].get_xlim()[0] > 0.0:
                    axs[j][l].set_xlim(left = 0.0)
                if axs[j][l].get_xlim()[1] < 1.0:
                    axs[j][l].set_xlim(right = 1.0)
                if axs[j][l].get_ylim()[0] > 0.0:
                    axs[j][l].set_ylim(bottom = 0.0)
                if axs[j][l].get_ylim()[1] < 1.0:
                    axs[j][l].set_ylim(top = 1.0)
            ...
            for l in range(len(metrics)):
                if j != l:
                    ...
                    if axs[j][l].get_ylim()[1] < 1.0:
                        axs[j][l].set_ylim(top = 1.0)
                    axs[j][l].legend(loc = "best")
    filename = os.path.join(dirname, "metrics/"+instance+".png")
    plt.savefig(filename, format = "png")
